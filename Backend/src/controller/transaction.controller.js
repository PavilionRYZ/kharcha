...
export const getTransactions = async (req, res) => {
  try {
    ...
    console.log("Error During Fetching Transactions:", error);
...
export const getBalanceSummary = async (req, res) => {
  try {
    ...
    const balanceSummary = await sql`
    ...
    res.status(200).json({
      balance: balanceSummary[0].balance,
      income: incomeResult[0].income,
      expense: expenseResult[0].expense,
    });
  } catch (error) {
    console.log("Error During Balance Summary:", error);
    res.status(500).json({ message: "Internal server error" });
  }
};
